 ...
 */
#ifndef TVM_CONTRIB_MXNET_CUDA_UTILS_H_
#define TVM_CONTRIB_MXNET_CUDA_UTILS_H_
...
mshadow::Tensor<xpu, ndim, DType> ToTensor(DLTensor* data) {
    CHECK_EQ(ndim, data->ndim);
    mshadow::Shape<ndim> shape;
    for (int i = 0; i < ndim; i++) {
        shape[i] = data->shape[i];
    }
    return mshadow::Tensor<xpu, ndim, DType>(
        reinterpret_cast<DType*>(data->data), std::move(shape));
}

template <typename T, typename F>
std::vector<T> to_const_vector(const tvm::Array<tvm::Expr>& arr, F fconvert) {
  std::vector<T> result;
...
inline std::vector<int> to_int_vector(const tvm::Array<tvm::Expr>& arr) {
  return to_const_vector<int>(arr, tvm::as_const_int);
}
...
#endif  // TVM_CONTRIB_MXNET_CUDA_UTILS_H_
